...
public final class StringSegment implements IReusable {
	...
	private StringSegment(String source, int offset, int length) {
		_source = source;
		_offset = offset;
		_length = length;
	}
	...
	private StringSegment createTrimmedSegment(int start, int end) throws Exception {
		...
		return obtain(_source, start, length);
	}
...
//		return _value;
	...
	private static Pool<StringSegment> pool = new Pool<StringSegment>(() -> {
