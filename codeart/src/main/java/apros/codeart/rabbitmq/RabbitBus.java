...
public class RabbitBus implements AutoCloseable {
    ...
    public void queueDelete(String queue) {
        try {
            Logger.trace("queueDelete_pre %s", queue);
            this.channel().queueDelete(queue);
            Logger.trace("queueDelete_ok %s", queue);
        } catch (Throwable ex) {
            throw propagate(ex);
        }
    }
...
//	#region 发布消息
     ...
     * @param setProperties
     */
    public void publish(String exchange, String routingKey, DTObject data,
...
//	#endregion
    ...
    private void messageReceived(String consumerTag, Delivery delivery) {
        ...
        var content = DTObject.readonly(delivery.getBody());
...
//		}
    ...
    public void cancel() {
        if (_consumerTag == null)
            return;
        try {
            this.channel().basicCancel(_consumerTag);   //取消消费者，不会删除队列
            _consumerTag = null;
        } catch (IOException e) {
            throw propagate(e);
        }
    ...
    private static final Function<Policy, Pool<RabbitBus>> _getPool = LazyIndexer.init((policy) -> {
