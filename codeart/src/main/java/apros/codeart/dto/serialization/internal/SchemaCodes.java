...
class SchemaCodes {
     ...
     * 成员名称 -> 架构代码
     */
    private final HashMap<String, String> _codes;

    /// <summary>
    /// 类型->架构代码
    /// </summary>
    private final HashMap<Class<?>, String> _typeCodes;

    private final DTObject _schema;

    private final boolean _containsAll;
    ...
    private void collectSchemaCode(DTEntity entity, Class<?> objectType) {
        ...
        if (TypeUtil.isCollection(objectType)) {
            ...
            if (pos > -1) {
                ...
                elementSC = StringUtil.trimStart(elementSC, "[");
                elementSC = StringUtil.trimEnd(elementSC, "]");
